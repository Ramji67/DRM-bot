...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "7657609554:AAEQsFaYWper4YpmVJqqJZkeIW7CUJ1lzS8")
    API_ID = int(os.environ.get("API_ID",  28088290))
    API_HASH = os.environ.get("API_HASH", "6998f2c585fdce65ac72dfa23d02b6ec")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '5850397219').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002360642973').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = 1002360642973
...
if __name__ == "__main__":
    ...
    async def main():
        await PRO.start()
        # h = await PRO.get_chat_member(chat_id= int(-1002120460486), user_id=1798290309)
        # print(h)
